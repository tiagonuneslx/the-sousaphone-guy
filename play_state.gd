...
func enter() -> void:
	...
	distance_px = 0
	Global.get_game().scroll_speed = Global.get_game().base_scroll_speed
	score = 0
...
func process(_delta: float) -> void:
	...
	Global.get_game().scroll_speed = Global.get_game().base_scroll_speed + pow(5*distance_px, .5)
	game.background.scroll_speed = Global.get_game().scroll_speed
	var distance_meters = distance_px * 5.0/16.0
	score = int(round(distance_meters))
	game.gui.distance_label.text = str(score) + "m"
	if(OS.has_feature('debug')):
		game.gui.debug_control_group.speed_label.text = "Speed: " + str(snapped(Global.get_game().scroll_speed * 5.0/16.0, 2)) + "m/s"
